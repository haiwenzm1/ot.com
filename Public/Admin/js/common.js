...
//提示
function alertTips(info, time, status) {
    status = status ? updateAlert(info, 'alert-success') : updateAlert(info);
    setTimeout(function () { $('.progress-bar').hide(); $('#top-alert').removeClass('block').slideUp(200); }, time);
}

//弹出层
function newMask(html){
    html = '<div id="mask-wrap"></div><div id="mask-content"><a id="mask-close" href="javascript:;" title="关闭"><i class="myicon r4c6"></i></a>' + html + '</div>';
    $(html).appendTo('#main-content');
    $(function () {
        $('#mask-close').mouseenter(function () {
            $(this).find('.r4c6').removeClass('r4c6').addClass('r4c5');
        }).mouseleave(function () {
            $(this).find('.r4c5').removeClass('r4c5').addClass('r4c6');
        }).click(function () {
            $('#mask-wrap,#mask-content').remove();
        })
    });
}
